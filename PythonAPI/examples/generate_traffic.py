...
def main():
    ...
    # 参数名称为 `--host`，这是在命令行中使用的完整参数形式，通常以双短划线开头，用于指定一个较为详细、明确的配置选项。
    # 在实际使用命令行运行程序时，用户可以通过 `--host <具体IP地址>` 的形式来传递对应的值，用于设置主机服务器的IP地址相关信息。
    '--host',
    # `metavar` 参数用于在生成的帮助信息中表示该参数对应的占位符名称，这里设置为 `H`，意味着当用户查看程序的帮助文档（比如通过 `python <程序名> -h` 命令）时，看到关于 `--host` 参数的提示部分，会显示类似 `H` 的占位形式，使得帮助信息更加清晰、规范，便于阅读和理解参数的使用方式。
    metavar='H',
    # `default` 参数指定了该参数的默认值为 `127.0.0.1`，也就是如果用户在命令行运行程序时没有明确指定 `--host` 参数的值，程序内部会自动将该参数视为 `127.0.0.1` 来进行后续操作。
    # `127.0.0.1` 是本地回环地址，常用于指代本地主机，这说明在默认情况下，程序会认为要连接的主机服务器就是本地这台机器本身。
    default='127.0.0.1',
    # `help` 参数提供了一段对该参数的解释性文本，用于帮助用户了解这个参数的具体作用和含义。
    # 这里明确说明了它用于设置主机服务器的IP地址，并且告知用户默认值是 `127.0.0.1`，让用户在需要指定不同的主机服务器IP时，知道如何通过命令行来进行相应配置。
